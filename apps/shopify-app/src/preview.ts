/**
 * Live banner preview for the Shopify authoring page.
 *
 * Same honest approach as the embed page and the Webflow Designer: run the REAL
 * runtime by pasting the shared {@link buildEmbedSnippet} output into a tiny
 * `<head>` and letting it boot in an `<iframe srcdoc>`. Two preview-only
 * overrides (`showMode:'everywhere'` + `consentModel:'opt-in'`) guarantee the
 * banner always shows and prompts; the config the page actually deploys is never
 * mutated.
 */
...
export function buildPreviewDocument(config: CookieConsentConfig): string {
  ...
  return [
    ...
    snippet,
    "</head><body>",
    '<main class="host"><h1>Your Shopify store</h1>',
