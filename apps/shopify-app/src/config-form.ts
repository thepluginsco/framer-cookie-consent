/**
 * Pure form-state → config mapping for the Shopify authoring UI.
 *
 * Identical in spirit to the Webflow Designer's and the embed page's
 * `config-from-form` — a flat, control-friendly projection of the config run
 * through the shared {@link mergeConfig} — so the page can never emit a config
 * the runtime can't read, and the app-embed block it generates is byte-identical
 * to every other platform's loader. Kept DOM-free so the "does the form produce
 * the right config" contract is unit-testable without a browser.
 */
...
/** Optional (non-`necessary`) default categories the page exposes. */
...
/** The flat state the Shopify form binds to. */
export interface ShopifyFormState {
  /* Content */
  title: string;
  message: string;
  acceptAll: string;
...
export function defaultFormState(): ShopifyFormState {
...
function categoriesFromForm(state: ShopifyFormState): CookieConsentConfig["categories"] {
...
function scriptsFromForm(state: ShopifyFormState): ManagedScript[] {
...
/** Map the flat {@link ShopifyFormState} to a complete, validated config. */
export function configFromForm(state: ShopifyFormState): CookieConsentConfig {
