/**
 * The Wix dashboard authoring UI (Phase 3.5 App shell, browser half).
 *
 * A framework-free authoring panel that runs as a Wix dashboard app: author the
 * banner, preview the real runtime, then install it on the current site via the
 * Data Client Worker (which holds the OAuth token AND the per-site config store).
 * All config logic lives in {@link configFromForm} (pure + tested); this file is
 * only DOM wiring + calling {@link WixDataClient}.
 */

import { buildEmbedSnippet } from "@framer-cookie-consent/shared";
import { WixDataClient } from "./data-client.js";
import {
  configFromForm,
  defaultFormState,
...
import { currentSiteId, inWixHost } from "./wix-host.js";
...
const WORKER_BASE =
  ...
  "https://consentful-wix.example.workers.dev";
...
export function mountApp(root: HTMLElement): void {
  ...
  const siteInput = el("input", { class: "cc-input", type: "text", placeholder: "Wix site id" }) as HTMLInputElement;
  const connectBtn = el("button", { class: "cc-btn", type: "button" }, ["Connect Wix"]) as HTMLButtonElement;
  const installBtn = el("button", { class: "cc-btn cc-btn--primary", type: "button" }, ["Install banner"]) as HTMLButtonElement;
  ...
  connectBtn.addEventListener("click", () => {
    ...
    resultLine.textContent = "Complete the Wix installation, then re-check the connection.";
  });
  ...
  installBtn.addEventListener("click", async () => {
    await run(installBtn, async () => {
      ...
      const bits: string[] = [];
      bits.push(r.changed ? "Banner installed." : "Config saved (banner already installed).");
      if (r.defaultPolicySet) bits.push("Wix consent default set to reject-all.");
      return bits.join(" ");
    });
  });
  ...
  // Resolve the site from the Wix host when present; else expose the manual input.
  void (async () => {
...
function buildHeader(): HTMLElement {
  return el("header", { class: "cc-header" }, [
    el("div", { class: "cc-header__brand" }, [
      ...
      el("span", { class: "cc-header__name" }, ["Consentful for Wix"]),
    ]),
    el("p", { class: "cc-header__tagline" }, [
      inWixHost()
        ? "Author your consent banner and install it on this Wix site."
        : "Preview mode — open inside the Wix dashboard to install to a site.",
    ]),
  ]);
}

/** The one-consent-app-per-site notice Wix's "Consent Apps" program requires. */
function buildNotice(): HTMLElement {
  return el("div", { class: "cc-notice" }, [
    el("strong", {}, ["Heads up: "]),
    "Wix allows only one consent app per site. Installing Consentful means turning " +
      "off Wix's built-in cookie banner (and any other consent app) first, so they " +
      "don't clash.",
  ]);
}
...
function buildConnectionBar(
  ...
  return el("div", { class: "cc-connbar" }, [
    ...
    field("Site id", siteInput, "Auto-filled inside the Wix dashboard"),
    connectBtn,
  ]);
}

function buildForm(state: WixFormState, refresh: () => void): HTMLElement {
...
function buildScriptsEditor(state: WixFormState, refresh: () => void): HTMLElement {
...
export function currentSnippet(state: WixFormState): string {
