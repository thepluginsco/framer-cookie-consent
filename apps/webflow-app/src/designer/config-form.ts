/**
 * Pure form-state → config mapping for the Webflow Designer Extension.
 *
 * Mirrors the embed page's `config-from-form.ts` in spirit — a flat,
 * control-friendly projection of the config run through the shared
 * {@link mergeConfig} — so the Designer can never emit a config the runtime can't
 * read, and the loader it installs is byte-identical to every other platform's.
 * Kept DOM-free so this "does the form produce the right config" contract is
 * unit-testable without a browser or the Webflow Designer host.
 */
...
/** Optional (non-`necessary`) default categories the Designer exposes. */
...
/** The flat state the Designer form binds to. */
export interface WebflowFormState {
  /* Content */
  title: string;
  message: string;
  acceptAll: string;
...
/** The optional categories the Designer exposes, in display order. */
...
export function defaultFormState(): WebflowFormState {
...
function categoriesFromForm(state: WebflowFormState): CookieConsentConfig["categories"] {
...
function scriptsFromForm(state: WebflowFormState): ManagedScript[] {
...
/** Map the flat {@link WebflowFormState} to a complete, validated config. */
export function configFromForm(state: WebflowFormState): CookieConsentConfig {
