...
export interface CfgScript {
  ...
  cat: string
  /**
   * Visitor-facing one-line purpose, shown in the preference center's per-vendor
   * list (Phase 4.2). Empty falls back to the provider host, so it is optional.
   */
  purpose: string
}
...
/**
 * One A/B banner variant in the design's flat shape. The five override fields
 * are "inherit when empty": a blank string (or `""` layout) means the variant
 * uses the base config's value, so authoring a variant only means filling the
 * bits you want to differ. Maps to the shared {@link AbVariant} (copy → `strings`,
 * layout → `banner`) on write.
 */
export interface CfgVariant {
  /** Stable id used to bucket visitors + tag analytics (e.g. `'A'`). */
  id: string
  /** Human label shown in the editor and Insights dashboard. */
  label: string
  /** Relative traffic weight (≥ 0). */
  weight: number
  /** Override for the banner heading (empty = inherit base). */
  heading: string
  /** Override for the banner body copy (empty = inherit base). */
  body: string
  /** Override for the accept-all label (empty = inherit base). */
  acceptLabel: string
  /** Override for the reject-all label (empty = inherit base). */
  rejectLabel: string
  /** Override for the banner layout (`""` = inherit base). */
  layout: BannerLayout | ""
}
...
export interface Cfg {
  ...
  languages: string[]
  /** A/B consent-rate test on (Pro). Needs ≥2 variants to actually run. */
  abTestEnabled: boolean
  /** The A/B banner variants under test (Phase 4.1). */
  abVariants: CfgVariant[]
  /** Preference center: list the individual vendors under each category (Phase 4.2). */
  showVendors: boolean
  /** Preference center: give each listed vendor its own on/off switch. */
...
export function toCfg(c: CookieConsentConfig): Cfg {
  return {
    ...
    // Editor hint only: the portal activation writes the tier; the published
    // runtime re-derives entitlement from a domain-scoped signed token.
    ...
    scripts: c.scripts.map((s) => ({ name: s.name, id: s.tagId, type: s.type, value: s.value, cat: s.category, purpose: s.purpose })),
    languages: Object.keys(c.strings.translations),
    abTestEnabled: c.abTest.enabled,
    abVariants: c.abTest.variants.map((v) => ({
      id: v.id,
      label: v.label,
      weight: v.weight,
      heading: v.strings.title ?? "",
      body: v.strings.message ?? "",
      acceptLabel: v.strings.acceptAll ?? "",
      rejectLabel: v.strings.rejectAll ?? "",
      layout: v.banner.layout ?? "",
    })),
    showVendors: c.preferenceCenter.showVendors,
    perVendorToggles: c.preferenceCenter.perVendorToggles,
  }
}

/**
 * Write the design A/B variants back into the canonical config. Pure; exported
 * for tests. Empty override fields are DROPPED (so the shared engine inherits the
 * base for them), ids are defaulted to `V1`, `V2`, … and weights are floored at 0.
 * Only copy + layout are emitted here — the schema's compliance-neutral variant
 * contract means these can never change what the banner blocks.
 */
export function applyAbTest(prev: CookieConsentConfig, enabled: boolean, variants: CfgVariant[]): CookieConsentConfig {
  const next: AbVariant[] = variants.map((v, i) => {
    const strings: AbVariant["strings"] = {}
    if (v.heading.trim()) strings.title = v.heading
    if (v.body.trim()) strings.message = v.body
    if (v.acceptLabel.trim()) strings.acceptAll = v.acceptLabel
    if (v.rejectLabel.trim()) strings.rejectAll = v.rejectLabel
    const banner: AbVariant["banner"] = {}
    if (v.layout) banner.layout = v.layout
    return {
      id: v.id.trim() || `V${i + 1}`,
      label: v.label,
  ...
  return { ...prev, abTest: { enabled, variants: next } }
}
...
export interface ConsentfulModel {
  ...
  createScript: (draft: { name: string; type: ScriptType; value: string; id: string; cat: string; purpose?: string }) => void
  ...
  /** Transform the A/B variant list (add / edit / remove). */
  setAbVariants: (fn: (variants: CfgVariant[]) => CfgVariant[]) => void
}
...
function applyScalar(prev: CookieConsentConfig, key: ScalarKey, value: unknown): CookieConsentConfig {
  ...
  switch (key) {
    ...
    case "customCss":
      ...
      break
    case "showVendors":
      next.preferenceCenter = { ...prev.preferenceCenter, showVendors: value as boolean }
      break
    case "perVendorToggles":
      next.preferenceCenter = { ...prev.preferenceCenter, perVendorToggles: value as boolean }
      break
    case "waitForUpdate":
...
export function useConsentful(): ConsentfulModel {
  ...
  const createScript = useCallback(
    (draft: { name: string; type: ScriptType; value: string; id: string; cat: string; purpose?: string }) => {
      const name = draft.name.trim()
      ...
      setScripts((scripts) => [
        ...scripts,
        { name, type: draft.type, value, id: draft.id.trim(), cat: draft.cat, purpose: (draft.purpose ?? "").trim() },
      ])
    },
    [setScripts],
  )
  ...
  const setAbVariants = useCallback(
    (fn: (variants: CfgVariant[]) => CfgVariant[]) => {
      update((prev) => {
        const cur = toCfg(prev)
        return applyAbTest(prev, cur.abTestEnabled, fn(cur.abVariants))
      })
    },
    [update],
  )
