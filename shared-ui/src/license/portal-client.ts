/**
 * Client for the Consentful licensing **portal** — activation from ANY
 * Consentful editor (Framer, Webflow, WordPress, Shopify, Wix, universal embed).
 *
 * Licensing is domain-based and portal-authoritative: the user pastes a license
 * key and the editor ACTIVATES the site's domain against it (creating/renewing a
 * "site seat").
...
export interface ActivationResult {
  ...
  reason: string | null
  /** Production sites the license allows (from the plan), or `null` if unknown. */
  maxSites: number | null
  /** Whether the domain is a free preview/staging host (no slot used). */
  isDev: boolean
}
  ...
  return {
    async activate(licenseKey, domain) {
      ...
      return {
        ...
        maxSites: ok && typeof body.maxSites === "number" ? body.maxSites : null,
        isDev: ok && body.isDev === true,
      }
    },
  }
}
